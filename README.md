...
Current tests are in `tests/test_main.py`, but they target a legacy `/analyze` route that is not present in current API routes.
...
If tests fail, verify route names and payloads against `app/main.py` and `api/routes.py`.
