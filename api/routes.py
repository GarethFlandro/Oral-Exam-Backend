...
def login(response: Response, login_request: LoginRequest):
    ...
    response.set_cookie(key="session_token", value=session_id, httponly=False, samesite="none", secure=True)
    response.set_cookie(key="API_KEY", value=ORAL_EXAM_API_KEY, httponly=False, samesite="none", secure=True)
    response.set_cookie(key="isTeacher", value=is_teacher, httponly=False, samesite="none", secure=True)
...
def logout(response: Response, session_token: str):
    ...
    response.delete_cookie("isTeacher")
